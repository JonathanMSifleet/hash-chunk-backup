...
foreach ($file in $files) {
    ...
    try {
        while (($bytesRead = $stream.Read($buffer, 0, $chunkSize)) -gt 0) {
            ...
            try {
                $hashBytes = $sha256.ComputeHash($chunkData)
            } finally {
                $sha256.Dispose()
            }
            ...
            if ($existingHash -ne $chunkHash) {
                ...
                Write-ChunkFile -targetDir $targetPath -fileName $file.Name -chunkName $chunkName -chunkData $chunkData `
